...
def print_student_report(student):
    ...
    if student['subjects']:
        ...
        for i in range(len(student['subjects'])):
            ...
            grade = student['grades'][i] if i < len(student['subjects']) else 'нет оценки'
            print(f"{subject}: {grade}")
    else:
        print('Нет предметов')
    print("="*40)

def add_subjects_and_grades(student):
    print("\n--- Добавление предметов ---")
    student['subjects'] = []
    student['grades'] = []
    while True:
        subjects = input("Введите предмет (или 'стоп')")
        if subjects.lower() == 'стоп':
            if len(student['subjects'] < 0):
                print("Добавьте хотя бы 1 предмет!")
                continue
            break
        if subject:
            student['subjects'].append(subject)
            # Ввод оценки по предмету (данному)
            while True:
                try:
                    grade = int(input(f"Оценка за {subject}: "))
                    if 2 <= grade <= 5:
                        student['grades'].append(grade)
                        print(f"Добавлен предмет: {subject} с оценкой {grade}")
                        break
                    else:
...
def analyze_perfomance(student):
    if not student['grade']:
        print('Нет оценок для анализа')
        return
    if len(student['subjencts']) != len(student['grades']):
        print('Кол-во предметов не соответствует кол-ву оценок')
        return
    # Самый успешный предмет
    max_grade = max(student['grades'])
    max_index = student['grades'].index(max_grade)
    best_subject = student['subjects'][max_index]
    # Самый сложный предмет
    min_grade = min(student['subjects'])
    min_index = student['subjects'].index(min_grade)
    worst_subjects = student['subjects'][min_grade]

    print('\n Анализ успеваемости:')
    print(f"Самый успешный предмет: {best_subject}")
    print(f"Самый сложный предмет: {worst_subjects}")

def ask_to_save(student):
